...
\begin{itemize}
  \item 在 navy-apps/apps/pal/src/global/global.c 的 PAL\_LoadGame()中通过 fread()读取游戏存档
  \item 在 navy-apps/apps/pal/src/hal/hal.c 的 redraw()中通过 NDL\_DrawRect()更新屏幕
\end{itemize}
请结合代码解释仙剑奇侠传,库函数,libos,Nanos-lite,AM,NEMU 是如何相互协助,来分别完成游戏存档的读取和屏幕的更新.
}

AM 从运行程序的视角刻画了一台计算机应该具备的功能，是对一台抽象计算机的概念定义，NEMU 通过程序来实现而不是电路对这台抽象计算机的具体实现。
...
该函数的参数传入的就是需要读取的存档名，然后调用fopen打开目的存档的文件。然后，我们使用是stdio.h的带缓冲的标准的IO库函数fread函数将存档中的信息读取到static saved类型的变量s中，调用这个带缓冲的函数，其在调用 fwrite 的时候，先把数据放在缓冲区。不直接使用系统调用，切换到内核态。而是等到缓冲区满等条件 满足时，再一次性调用，把数据拷贝到内核空间，这样减少使用 read 和 write 调用的次数，也就是用户态和内核态切换的次数。
